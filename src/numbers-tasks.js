...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const radPosit = (x2 - x1) ** 2;
  const axiPosit = (y2 - y1) ** 2;
  return Math.sqrt(radPosit + axiPosit);
}
...
function toNumber(value, def) {
  ...
  const isNotNumber = Number.isNaN(valueToNumber);
  return isNotNumber ? def : valueToNumber;
}
...
function getFibonacciNumber(index) {
  ...
  for (let id = 2; id < index + 1; id += 1) {
    result.push(result[id - 2] + result[id - 1]);
  }
